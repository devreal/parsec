...
static void parsec_termdet_threadcount_monitor_taskpool(parsec_taskpool_t *tp,
                                                        parsec_termdet_termination_detected_function_t cb);
static parsec_termdet_taskpool_state_t parsec_termdet_threadcount_taskpool_state(parsec_taskpool_t *tp);
static int parsec_termdet_threadcount_taskpool_ready(parsec_taskpool_t *tp);
static int parsec_termdet_threadcount_taskpool_addto_nb_tasks(parsec_taskpool_t *tp, int v);
static int parsec_termdet_threadcount_taskpool_addto_nb_pa(parsec_taskpool_t *tp, int v);
static int parsec_termdet_threadcount_taskpool_set_nb_tasks(parsec_taskpool_t *tp, int v);
static int parsec_termdet_threadcount_taskpool_set_nb_pa(parsec_taskpool_t *tp, int v);

static int parsec_termdet_threadcount_outgoing_message_pack(parsec_taskpool_t *tp,
                                                            int dst_rank,
                                                            char *packed_buffer,
                                                            int *position,
                                                            int buffer_size);
static int parsec_termdet_threadcount_outgoing_message_start(parsec_taskpool_t *tp,
                                                            int dst_rank,
                                                            parsec_remote_deps_t *remote_deps);
static int parsec_termdet_threadcount_incoming_message_start(parsec_taskpool_t *tp,
                                                             int src_rank,
                                                             char *packed_buffer,
                                                             int *position,
                                                             int buffer_size,
...
typedef struct threadcount_monitor_s threadcount_monitor_t;

const parsec_termdet_module_t parsec_termdet_threadcount_module = {
    &parsec_termdet_threadcount_component,
    {
        parsec_termdet_threadcount_monitor_taskpool,
        parsec_termdet_threadcount_taskpool_state,
        parsec_termdet_threadcount_taskpool_ready,
        parsec_termdet_threadcount_taskpool_addto_nb_tasks,
        parsec_termdet_threadcount_taskpool_addto_nb_pa,
        parsec_termdet_threadcount_taskpool_set_nb_tasks,
        parsec_termdet_threadcount_taskpool_set_nb_pa,
        0,
        parsec_termdet_threadcount_outgoing_message_start,
...
typedef enum {
    PARSEC_TERMDET_THREADCOUNT_NOT_READY,
    PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_CHILDREN,
    PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT,
    PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_CHILDREN,
    PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT,
} parsec_termdet_threadcount_state_t;
...
typedef struct parsec_termdet_threadcount_monitor_s {
    ...
    parsec_termdet_threadcount_state_t state;   /**< Current status */
...
static void parsec_termdet_threadcount_msg_down(parsec_termdet_threadcount_msg_down_t *msg, int src, parsec_taskpool_t *tp);
static void parsec_termdet_threadcount_msg_up(parsec_termdet_threadcount_msg_up_t *msg, int src, parsec_taskpool_t *tp);

parsec_list_t parsec_termdet_threadcount_delayed_messages;

static int parsec_termdet_threadcount_msg_dispatch_taskpool(parsec_taskpool_t *tp, parsec_comm_engine_t *ce,
                                                            long unsigned int tag,  void *msg,
                                                            long unsigned int size, int src,  void *module)
{
    parsec_termdet_threadcount_msg_type_t t = *(parsec_termdet_threadcount_msg_type_t*)msg;
    parsec_termdet_threadcount_msg_down_t *down_msg = (parsec_termdet_threadcount_msg_down_t*)msg;
    parsec_termdet_threadcount_msg_up_t *up_msg = (parsec_termdet_threadcount_msg_up_t*)msg;
    (void)size;
    (void)tag;
    (void)module;
    (void)ce;

    parsec_list_unlock(&parsec_termdet_threadcount_delayed_messages);
    ...
    switch( t ) {
    case PARSEC_TERMDET_THREADCOUNT_MSG_TYPE_DOWN:
        assert( size == sizeof(parsec_termdet_threadcount_msg_down_t) );
        PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tIt is a DOWN message with result %d",
                             down_msg->result);
        parsec_termdet_threadcount_msg_down( down_msg, src, tp );
        return PARSEC_SUCCESS;

    case PARSEC_TERMDET_THREADCOUNT_MSG_TYPE_UP:
        assert( size == sizeof(parsec_termdet_threadcount_msg_up_t) );
        ...
        parsec_termdet_threadcount_msg_up( up_msg, src, tp );
        return PARSEC_SUCCESS;
    }
    assert(0);
    return PARSEC_ERROR;
...
int parsec_termdet_threadcount_msg_dispatch(parsec_comm_engine_t *ce, long unsigned int tag,  void *msg, long unsigned int size, int src,  void *module)
{
    parsec_termdet_threadcount_delayed_msg_t *delayed_msg;
    parsec_termdet_threadcount_msg_down_t *down_msg = (parsec_termdet_threadcount_msg_down_t*)msg;
    parsec_taskpool_t *tp = parsec_taskpool_lookup(down_msg->tp_id);

    if( (NULL == tp) || (NULL == tp->tdm.monitor) ||
        (((parsec_termdet_threadcount_monitor_t*)tp->tdm.monitor)->state == PARSEC_TERMDET_THREADCOUNT_NOT_READY) ) {
        parsec_list_lock(&parsec_termdet_threadcount_delayed_messages);
        ...
        if ((NULL == tp) || (NULL == tp->tdm.monitor) ||
            (((parsec_termdet_threadcount_monitor_t *) tp->tdm.monitor)->state ==
             PARSEC_TERMDET_THREADCOUNT_NOT_READY)) {
            delayed_msg = (parsec_termdet_threadcount_delayed_msg_t *) malloc(
                    sizeof(parsec_termdet_threadcount_delayed_msg_t));
            PARSEC_LIST_ITEM_SINGLETON(delayed_msg);
            assert(size <= PARSEC_TERMDET_THREADCOUNT_MAX_MSG_SIZE);
            delayed_msg->ce = ce;
            ...
            parsec_list_nolock_push_back(&parsec_termdet_threadcount_delayed_messages, &delayed_msg->list_item);
            parsec_list_unlock(&parsec_termdet_threadcount_delayed_messages);
            return PARSEC_SUCCESS;
        }
        parsec_list_unlock(&parsec_termdet_threadcount_delayed_messages);
    ...
    return parsec_termdet_threadcount_msg_dispatch_taskpool(tp, ce, tag,  msg, size, src,  module);
}


static int parsec_termdet_threadcount_topology_nb_children(parsec_taskpool_t *tp)
{
...
static int parsec_termdet_threadcount_topology_is_root(parsec_taskpool_t *tp)
{
    parsec_context_t *context;
    assert(tp->context != NULL);
    context = tp->context;
...
static int parsec_termdet_threadcount_topology_child(parsec_taskpool_t *tp, int i)
{
    parsec_context_t *context;
    assert(tp->context != NULL);
    context = tp->context;
...
static int parsec_termdet_threadcount_topology_parent(parsec_taskpool_t *tp)
{
    parsec_context_t *context;
    assert(tp->context != NULL);
    context = tp->context;
                                                        ...
                                                        parsec_termdet_termination_detected_function_t cb)
{
    ...
    tpm->state = PARSEC_TERMDET_THREADCOUNT_NOT_READY;
...
{
    parsec_termdet_threadcount_monitor_t *tpm;
    parsec_termdet_threadcount_state_t state;
...
recheck:
    ...
    switch(state) {
    case PARSEC_TERMDET_THREADCOUNT_NOT_READY:
        return PARSEC_TERM_TP_NOT_READY;
    case PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_CHILDREN:
    case PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT:
        return PARSEC_TERM_TP_BUSY;
    case PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_CHILDREN:
    case PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT:
...
static int parsec_termdet_threadcount_taskpool_ready(parsec_taskpool_t *tp)
{
    ...
    parsec_termdet_threadcount_delayed_msg_t *delayed_msg;
    parsec_termdet_threadcount_msg_down_t *down_msg;

    assert( tp->tdm.module != NULL );
    assert( tp->tdm.module == &parsec_termdet_threadcount_module.module );
    assert( tp->tdm.monitor != PARSEC_TERMDET_THREADCOUNT_TERMINATED );
    tpm = (parsec_termdet_threadcount_monitor_t*)tp->tdm.monitor;
    assert( tpm->state == PARSEC_TERMDET_THREADCOUNT_NOT_READY );
    parsec_atomic_rwlock_wrlock(&tpm->rw_lock);
    tpm->nb_child_left = parsec_termdet_threadcount_topology_nb_children(tp);
    tpm->state = PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_CHILDREN; /* This is true even if nb_children == 0:
    ...
    parsec_list_lock(&parsec_termdet_threadcount_delayed_messages);
    for(item = PARSEC_LIST_ITERATOR_FIRST(&parsec_termdet_threadcount_delayed_messages);
        item != PARSEC_LIST_ITERATOR_END(&parsec_termdet_threadcount_delayed_messages);
        item = next) {
        next = PARSEC_LIST_ITEM_NEXT(item);
        delayed_msg = (parsec_termdet_threadcount_delayed_msg_t*)item;
        down_msg = (parsec_termdet_threadcount_msg_down_t*)delayed_msg->msg;
        if(down_msg->tp_id == tp->taskpool_id) {
            parsec_list_nolock_remove(&parsec_termdet_threadcount_delayed_messages, item);
            parsec_termdet_threadcount_msg_dispatch_taskpool(tp, delayed_msg->ce, delayed_msg->tag,
...
static void parsec_termdet_threadcount_send_up_messages(parsec_termdet_threadcount_monitor_t *tpm,
                                                        parsec_taskpool_t *tp)
{
    parsec_termdet_threadcount_msg_up_t msg_up;
    parsec_termdet_threadcount_msg_down_t msg_down;
    int i;
    ...
    if( parsec_termdet_threadcount_topology_is_root(tp) ) {
        msg_down.msg_type = PARSEC_TERMDET_THREADCOUNT_MSG_TYPE_DOWN;
        msg_down.tp_id = tp->taskpool_id;
        if( parsec_termdet_threadcount_topology_nb_children(tp) == 0 ) {
        ...
        for(i = 0; i < parsec_termdet_threadcount_topology_nb_children(tp); i++) {
            PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tSending DOWN message with result %d to rank %d. Justification: last_acc_sent_at_root = %d, acc_sent = %d, last_acc_received_at_root = %d, acc_received = %d",
                                 msg_down.result, parsec_termdet_threadcount_topology_child(tp, i),
                                 tpm->last_acc_sent_at_root, tpm->acc_sent, tpm->last_acc_received_at_root, tpm->acc_received);
            tpm->stats_nb_sent_msg++;
            tpm->stats_nb_sent_bytes += sizeof(parsec_termdet_threadcount_msg_down_t) + sizeof(int);
            parsec_ce.send_am(&parsec_ce, PARSEC_TERMDET_THREADCOUNT_MSG_TAG, parsec_termdet_threadcount_topology_child(tp, i), &msg_down, sizeof(parsec_termdet_threadcount_msg_down_t));
        }
        ...
        if( msg_down.result ) {
            ...
            tp->tdm.monitor = PARSEC_TERMDET_THREADCOUNT_TERMINATED;
            tp->tdm.callback(tp);
            free(tpm);
        } else {
            tpm->acc_sent = 0;
        ...
        tpm->state = PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT;
        PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tProcess changed state for IDLE_WAITING_FOR_PARENT");
        msg_up.msg_type = PARSEC_TERMDET_THREADCOUNT_MSG_TYPE_UP;
        ...
        PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tSending UP message with nb_sent / nb_received of %d/%d to rank %d",
                             msg_up.nb_sent, msg_up.nb_received, parsec_termdet_threadcount_topology_parent(tp));
        tpm->stats_nb_sent_msg++;
        tpm->stats_nb_sent_bytes += sizeof(parsec_termdet_threadcount_msg_up_t) + sizeof(int);
        parsec_ce.send_am(&parsec_ce, PARSEC_TERMDET_THREADCOUNT_MSG_TAG, parsec_termdet_threadcount_topology_parent(tp), &msg_up, sizeof(parsec_termdet_threadcount_msg_up_t));
    }
}

static void parsec_termdet_threadcount_check_state_message_received(parsec_termdet_threadcount_monitor_t *tpm,
                                                                    parsec_taskpool_t *tp)
{
    if(tp->nb_tasks == 0 &&
       tp->nb_pending_actions == 0 &&
       tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_CHILDREN &&
       tpm->nb_child_left == 0) {
        parsec_termdet_threadcount_send_up_messages(tpm, tp);
    }
}
                                                                    ...
                                                                    parsec_taskpool_t *tp)
{
    ...
    if(tp->nb_tasks == 0 && tp->nb_pending_actions == 0) {
        ...
        if( tpm->state == PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT) {
            PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tProcess changed state for IDLE_WAITING_FOR_PARENT");
            tpm->state = PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT;
            tpm->stats_nb_busy_idle++;
        } else if( tpm->state == PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_CHILDREN ) {
        ...
        /* We are now BUSY */
        if( tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_CHILDREN ) {
            PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tProcess changed state for BUSY_WAITING_FOR_CHILDREN");
            tpm->state = PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_CHILDREN;
            tpm->stats_nb_idle_busy++;
        } else if (tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT ) {
            PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tProcess changed state for BUSY_WAITING_FOR_PARENT");
            tpm->state = PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT;
            tpm->stats_nb_idle_busy++;
        }
    }
}
...
static void parsec_termdet_threadcount_msg_down(parsec_termdet_threadcount_msg_down_t *msg, int src, parsec_taskpool_t *tp)
{
    ...
    assert(&parsec_termdet_threadcount_module.module == tp->tdm.module);
    assert(PARSEC_TERMDET_THREADCOUNT_TERMINATED != tp->tdm.monitor);
    tpm = (parsec_termdet_threadcount_monitor_t *)tp->tdm.monitor;

    parsec_atomic_rwlock_wrlock(&tpm->rw_lock);
    tpm->stats_nb_recv_msg++;
    tpm->stats_nb_recv_bytes += sizeof(parsec_termdet_threadcount_msg_down_t)+sizeof(int);
    assert(tpm->state == PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT ||
           tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT );
    assert((int)tpm->nb_child_left == parsec_termdet_threadcount_topology_nb_children(tp));

    for(i = 0; i < parsec_termdet_threadcount_topology_nb_children(tp); i++) {
        PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tSending DOWN message with result %d to rank %d",
                             msg->result, parsec_termdet_threadcount_topology_child(tp, i));
        tpm->stats_nb_sent_msg++;
        tpm->stats_nb_sent_bytes += sizeof(parsec_termdet_threadcount_msg_down_t) + sizeof(int);
        parsec_ce.send_am(&parsec_ce, PARSEC_TERMDET_THREADCOUNT_MSG_TAG, parsec_termdet_threadcount_topology_child(tp, i), msg, sizeof(parsec_termdet_threadcount_msg_down_t));
    }

    if(msg->result) {
        assert(tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT);
        gettimeofday(&tpm->stats_time_end, NULL);
        tp->tdm.monitor = PARSEC_TERMDET_THREADCOUNT_TERMINATED;
        ...
        tpm->acc_sent = 0;
        tpm->acc_received = 0;
        if( tpm->state == PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_PARENT ) {
            tpm->state = PARSEC_TERMDET_THREADCOUNT_IDLE_WAITING_FOR_CHILDREN;
            PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "TERMDET-4C:\tChange state to IDLE_WAITING_FOR_CHILDREN on DOWN(false) message");
            parsec_termdet_threadcount_check_state_message_received(tpm, tp); /* In case tpm->nb_child_left is 0 already */
        } else {
            assert(tpm->state == PARSEC_TERMDET_THREADCOUNT_BUSY_WAITING_FOR_PARENT);
...
static void parsec_termdet_threadcount_msg_up(parsec_termdet_threadcount_msg_up_t *msg, int src, parsec_taskpool_t *tp)
{
    ...
    parsec_atomic_rwlock_wrlock(&tpm->rw_lock);
    tpm->stats_nb_recv_msg++;
    tpm->stats_nb_recv_bytes += sizeof(parsec_termdet_threadcount_msg_up_t)+sizeof(int);
