...
static int parsec_composed_cb( parsec_taskpool_t* o, void* cbdata )
{
    ...
    remaining = compound->tdm.module->taskpool_addto_nb_pa(compound, -1);
    if( remaining > 0 ) {
        ...
        PARSEC_DEBUG_VERBOSE(30, parsec_debug_output, "Compound taskpool completed %p",
                             compound);
        parsec_taskpool_termination_detected(compound);
    }
    return 0;
}
                                     ...
                                     parsec_task_t** startup_list )
{
    ...
    compound_tp->tdm.module->taskpool_set_nb_pa(compound_tp, compound_state->nb_taskpools);
    PARSEC_DEBUG_VERBOSE(30, parsec_debug_output, "Compound taskpool %p starting with %d taskpools",
                         compound_tp, compound_state->nb_taskpools);
