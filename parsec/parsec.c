...
{
    if( tc->task_class_id >= tp->nb_task_classes ) {
        /* we need to resize the array large enough to account for the new task class */
        int i, length = tp->nb_task_classes + 8;
        ...
        for( i = tp->nb_task_classes; i < length; i++ )
            tp->task_classes_array[i] = NULL;
        tp->nb_task_classes = length;
    }
